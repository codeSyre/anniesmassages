...
const userMenuTrigger = document.querySelector<HTMLElement>('[data-user-menu-trigger]');
...
const incomeTrendForm = document.querySelector<HTMLFormElement>('[data-income-trend-filter]');
const incomeTrendMonthInput = document.querySelector<HTMLInputElement>('[data-income-trend-month]');
...
const incomeTrendChartShell = document.querySelector<HTMLElement>('[data-income-trend-chart]');

if (incomeTrendChartShell !== null) {
  const chartSvg = incomeTrendChartShell.querySelector<SVGSVGElement>('.income-trend-chart');
  const hoverGuide = incomeTrendChartShell.querySelector<SVGLineElement>('[data-income-trend-hover-guide]');
  const tooltip = incomeTrendChartShell.querySelector<HTMLElement>('[data-income-trend-tooltip]');
  const tooltipValue = incomeTrendChartShell.querySelector<HTMLElement>('[data-income-trend-tooltip-value]');
  const tooltipLabel = incomeTrendChartShell.querySelector<HTMLElement>('[data-income-trend-tooltip-label]');
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      points = parsed.filter((point): point is IncomeTrendPoint => {
        if (typeof point !== 'object' || point === null) {
          return false;
        }

        const candidate = point as Partial<IncomeTrendPoint>;
  ...
  if (
    ...
    const updateIncomeTrendTooltip = (event: MouseEvent) => {
...
const rowLinks = document.querySelectorAll<HTMLElement>('[data-row-link][data-row-link-href]');
